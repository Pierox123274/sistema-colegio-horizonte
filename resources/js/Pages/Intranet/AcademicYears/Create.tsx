...
export default function AcademicYearsCreate() {
    const form = useForm<AcademicYearFormState>({
        name: '',
        year: String(new Date().getFullYear()),
        starts_at: `${new Date().getFullYear()}-03-01`,
        ends_at: `${new Date().getFullYear()}-12-15`,
        is_active: true,
    });

    const submit: FormEventHandler = (e) => {
        e.preventDefault();
        form.post(route('intranet.academic-years.store'));
    };

    return (
        <IntranetLayout title="Nuevo año académico">
            <Head title="Nuevo año académico — Horizonte" />

            <PageContainer>
                <SectionTitle
                    title="Registrar año académico"
                    description="El año numérico debe ser único. Si marca «Activo», los demás años quedarán inactivos."
                    actions={
                        <Link
