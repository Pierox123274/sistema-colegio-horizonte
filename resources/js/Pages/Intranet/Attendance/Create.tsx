...
type P = PageProps<{ catalog: Catalog; batch: Batch }>;

export default function AttendanceCreate() {
    const { catalog, batch } = usePage<P>().props;
    ...
    const loadStudents = () => {
        ...
        router.get(route('intranet.attendance.section-date', { date, section: sectionId }), { academic_year_id: academicYearId || undefined }, { preserveState: false });
    };
    ...
    return (
        <IntranetLayout title="Asistencia - Registrar">
            <Head title="Asistencia - Registrar" />
            <PageContainer>
                <IntranetBreadcrumbs items={[{ label: 'Asistencia' }, { label: 'Registrar asistencia' }]} />
                <SectionTitle
                    title="Registro masivo de asistencia"
                    description="Selecciona año, fecha y sección; luego marca estado por estudiante matriculado."
                    actions={<Link href={route('intranet.attendance.index')} className="rounded-lg border border-plomo/20 px-4 py-2 text-sm font-semibold">Historial</Link>}
                />
                ...
                {batch ? (
                    <Card>
                        ...
                        <form onSubmit={(e) => {
                            e.preventDefault();
                            router.post(route('intranet.attendance.store'), {
                                ...form.data,
                                academic_year_id: academicYearId,
                                educational_level_id: levelId,
                                grade_id: gradeId,
                    ...
                    <Card><p className="text-sm text-plomo">Selecciona sección y fecha para cargar estudiantes matriculados.</p></Card>
                )}
            </PageContainer>
        </IntranetLayout>
    );
