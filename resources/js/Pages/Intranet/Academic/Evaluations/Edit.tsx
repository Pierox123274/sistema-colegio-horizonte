...
type Props = PageProps<{ catalog: Catalog; evaluation: any }>;

export default function EvaluationEdit() {
    const { catalog, evaluation } = usePage<Props>().props;
    const [levelId, setLevelId] = useState(String(evaluation.educational_level_id));
    const [gradeId, setGradeId] = useState(String(evaluation.grade_id));
    ...
    const form = useForm({
        ...
        academic_year_id: String(evaluation.academic_year_id),
        educational_level_id: String(evaluation.educational_level_id),
        grade_id: String(evaluation.grade_id),
        section_id: String(evaluation.section_id),
        title: evaluation.title,
        period: evaluation.period,
        evaluated_at: evaluation.evaluated_at,
        max_score: String(evaluation.max_score),
        weight: String(evaluation.weight),
        is_active: Boolean(evaluation.is_active),
    });

    return (
        <IntranetLayout title="Editar evaluación">
            <Head title="Editar evaluación" />
            <PageContainer>
                <IntranetBreadcrumbs items={[{ label: 'Gestión académica' }, { label: 'Evaluaciones' }, { label: 'Editar' }]} />
                <SectionTitle title="Editar evaluación" description="Actualiza configuración de la evaluación." actions={<Link href={route('intranet.academic.evaluations.index')} className="rounded-lg border border-plomo/20 px-4 py-2 text-sm font-semibold">Volver</Link>} />
                <Card>
                    <form className="grid gap-4 md:grid-cols-2" onSubmit={(e) => {
                        e.preventDefault();
                        form.put(route('intranet.academic.evaluations.update', evaluation.id));
                    }}>
                        ...
                        <div className="md:col-span-2"><button type="submit" disabled={form.processing} className="rounded-lg bg-navy-900 px-4 py-2 text-sm font-semibold text-white disabled:opacity-50">Guardar cambios</button></div>
                    </form>
                </Card>
            </PageContainer>
        </IntranetLayout>
