...
export default function GradesShow() {
    const { grade, permissions, flash } = usePage<ShowPageProps>().props;

    const sections = grade.sections ?? [];

    return (
        <IntranetLayout title={grade.name}>
            <Head title={`${grade.name} — Grado — Horizonte`} />
            ...
            <PageContainer>
                ...
                <SectionTitle
                    title={grade.name}
                    description={`${grade.educational_level?.code ?? ''} · Orden ${grade.order}`}
                    actions={
                        <div className="flex flex-wrap gap-2">
                            ...
                            >
                                Ver secciones filtradas
                            </Link>
                            {grade.educational_level ? (
                                <Link
                                    href={route(
                                        'intranet.academic.levels.show',
                                        grade.educational_level.id,
                                    )}
                                    className="inline-flex items-center rounded-lg border border-plomo/20 bg-white px-4 py-2 text-sm font-semibold text-navy-900 shadow-sm transition hover:bg-navy-50"
                                >
                                    Ver nivel
                    ...
                    <dl className="mt-4 grid gap-4 sm:grid-cols-2">
                        <div>
                            ...
                            <dd className="mt-1 text-lg font-bold text-navy-900">
                                {grade.sections_count ?? sections.length}
                            </dd>
                        </div>
                    </dl>
                </Card>
                ...
                <TableContainer
                    title="Secciones"
                    description="Divisiones del grado."
                >
                    {sections.length === 0 ? (
