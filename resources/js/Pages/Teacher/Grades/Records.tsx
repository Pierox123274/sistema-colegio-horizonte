...
export default function TeacherGradesRecords() {
    ...
    return (
        <TeacherLayout title="Registro de notas">
            <Head title="Portal docente — Registro de notas" />
            <PageContainer>
                <SectionTitle
                    title="Registro de notas"
                    description="Cargue notas por evaluación para estudiantes de sus secciones asignadas."
                    actions={
                        <Link href={route('teacher.grades.index')} className="rounded-lg border border-plomo/20 px-4 py-2 text-sm font-semibold text-navy-900">
                            Volver a notas
                        </Link>
                    }
                />
                <Card className="mb-6">
                    <div className="grid gap-3 md:grid-cols-5">
                        <select defaultValue={filters.academic_year_id} onChange={(e) => router.get(route('teacher.grades.records'), { ...filters, academic_year_id: e.target.value }, { preserveState: true })} className="rounded-md border border-plomo/20 px-3 py-2 text-sm"><option value="">Año académico</option>{catalog.academic_years.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select>
                        <select defaultValue={filters.section_id} onChange={(e) => router.get(route('teacher.grades.records'), { ...filters, section_id: e.target.value }, { preserveState: true })} className="rounded-md border border-plomo/20 px-3 py-2 text-sm"><option value="">Sección</option>{catalog.sections.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select>
                        <select defaultValue={filters.subject_id} onChange={(e) => router.get(route('teacher.grades.records'), { ...filters, subject_id: e.target.value }, { preserveState: true })} className="rounded-md border border-plomo/20 px-3 py-2 text-sm"><option value="">Curso</option>{catalog.subjects.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select>
                        <select defaultValue={filters.evaluation_id} onChange={(e) => router.get(route('teacher.grades.records'), { ...filters, evaluation_id: e.target.value }, { preserveState: true })} className="rounded-md border border-plomo/20 px-3 py-2 text-sm"><option value="">Evaluación</option>{catalog.evaluations.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select>
                        <div className="flex gap-2">
                            <a href={`${route('teacher.reports.grades.pdf')}?${new URLSearchParams(filters).toString()}`} className="rounded-md border border-plomo/20 px-3 py-2 text-sm font-semibold">PDF</a>
                            <a href={`${route('teacher.reports.grades.excel')}?${new URLSearchParams(filters).toString()}`} className="rounded-md border border-plomo/20 px-3 py-2 text-sm font-semibold">CSV</a>
                        </div>
                    </div>
                </Card>
                ...
                {batch && selected_evaluation ? (
                    <Card>
                        ...
                        {batch.students.length === 0 ? (
                            ...
                            <form onSubmit={(e) => {
                                e.preventDefault();
                                form.post(route('teacher.grades.records.store'));
                            }}>
                                    ...
                                    {batch.students.map((student, idx) => (
                                        <div key={student.id} className="grid gap-3 rounded-lg border border-plomo/10 p-3 md:grid-cols-[2fr_1fr_2fr_auto]">
                                            ...
                                            <div className="flex items-center">
                                                <Link href={route('teacher.students.show', student.id)} className="rounded border border-plomo/20 px-2 py-1 text-xs font-semibold">Ficha</Link>
                                            </div>
                                        </div>
                                    ))}
                                </div>
